...
{
    public partial class VeritabanıYonetim : Form
    {
        public VeritabanıYonetim()
        {
            InitializeComponent();
        ...
        string Yetki;
        public VeritabanıYonetim(string yetki)
        {
            InitializeComponent();
            Yetki = yetki;
        ...
        private void Form4_Load(object sender, EventArgs e)
        {
           // this.WindowState = FormWindowState.Maximized;
            ...
            SendMessage(textBox3.Handle, EM_SETCUEBANNER, 0, "Ad, Yatırımcı");

        }
                
        private void yatirimGrid()
        {
            try
            {
                baglan.Open();
                SqlCommand cmd = new SqlCommand("select y.proje_ID, y.proje_Adi,Y.proje_Yatirimci,y.proje_Adres,ISNULL( t.TedSayisi,0) as TedarikciSayisi,ISNULL( m.makinesayisi/t.TedSayisi,0) as MakineSayisi from proje_Yatirim as Y full join (select proje_ID,COUNT(proje_ID) as TedSayisi from proje_Tedarikciler group by proje_ID ) as T on Y.proje_ID=t.proje_ID full join (select proje_ID,COUNT(proje_ID) as makinesayisi from proje_Makine_Ekipman group by proje_ID) as M on M.proje_ID=Y.proje_ID", baglan);
        ...
        {
            if (dataGridView3.Rows.Count != 0 && dataGridView3.Rows.Count != 1)
            {
        ...
        {
            if (dataGridView4.Rows.Count != 0 && dataGridView4.Rows.Count != 1)
            {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                    int silinecek_Tedarikci_ID;
                    int.TryParse(dataGridView4.SelectedCells[0].Value.ToString().TrimEnd(), out silinecek_Tedarikci_ID);
                    if (tedarikciSilinebilirligi(silinecek_Tedarikci_ID) == true)
                    {
                        baglan.Open();
                        ...
                        MessageBox.Show("Bu Tedarikci Bir Yatırıma Kayıtlıdır. Silmek Yatırım Belgeleri Alınmak İstendiğinde Veri Kaybına Neden Olacağından Silinemez.", "Tedarikci Silme Uyarısı", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private bool tedarikciSilinebilirligi(int tedarikciID)
        {

            try
            {
                baglan.Open();
                SqlCommand cmd = new SqlCommand("select tedarikci_ID from proje_Tedarikciler group by tedarikci_ID", baglan);
                SqlDataReader dr = cmd.ExecuteReader();

                while (dr.Read())
                {
                    if (tedarikciID == Convert.ToInt32(dr["tedarikci_ID"].ToString().TrimEnd()))
                    {
                        baglan.Close();
                        return false;
                    }
        ...
        private void textBox3_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                (dataGridView1.DataSource as DataTable).DefaultView.RowFilter = string.Format("proje_Adi LIKE '{0}%' OR proje_Yatirimci LIKE '{0}%'", textBox3.Text);
            }
        }

        private void button11_Click(object sender, EventArgs e)
        {
            TKDK yeniEkle = new TKDK(Yetki);
            this.Close();
            yeniEkle.ShowDialog();
        }
        ...
        {
            DialogResult onay = new DialogResult();
            onay = MessageBox.Show("Yatırımı Silerseniz Bu Yatırıma Bağlı Tedarikçi,Makine Ekipman Ve Sartname Kayıtlarınıda Silmiş Olursunuz?\n-Tedarikçi ve Makine Ekipman Veritabanından Silinmez Bu Proje İle Bağlantısı Silinir.", "Silme Uyarısı", MessageBoxButtons.YesNo);
            if (onay == DialogResult.Yes)
            {
                try
                {
                    int silinecek_Yatirim_ID = Convert.ToInt32(dataGridView1.SelectedCells[0].Value.ToString().TrimEnd());
                   // string silinecek_Yatirim_Adi = dataGridView1.SelectedCells[1].Value.ToString().TrimEnd();
                    baglan.Open();
                    SqlCommand cmd = new SqlCommand("Delete from proje_Yatirim where proje_ID=" + silinecek_Yatirim_ID + " ", baglan);
                    cmd.ExecuteNonQuery();
                    baglan.Close();
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                int projeID = Convert.ToInt32(dataGridView1.SelectedCells[0].Value.ToString().TrimEnd());
                yatirimGuncelle yG = new yatirimGuncelle(projeID);
                yG.ShowDialog();
            }
            else
                MessageBox.Show("Düzenlenecek Kayıt Yok");
